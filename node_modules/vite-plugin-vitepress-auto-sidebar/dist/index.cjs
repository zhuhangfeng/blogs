...
function createSideBarItems(targetPath, ...reset) {
  ...
  const rawNode = fs.readdirSync(path.join(targetPath, ...reset));
  const node = beforeCreateSideBarItems?.(rawNode) ?? rawNode;
  const currentDir = path.join(targetPath, ...reset);
  if (ignoreIndexItem && node.length === 1 && node[0] === "index.md") {
    return [];
  }
  const result = [];
  for (const fname of node) {
    if (fs.statSync(path.join(targetPath, ...reset, fname)).isDirectory()) {
      if (ignoreList.some((item) => item === fname || item instanceof RegExp && item.test(fname))) {
        continue;
      }
      const items = createSideBarItems(path.join(targetPath), ...reset, fname);
      let text = fname;
      if (titleFromFile) {
        const filenames = [
      ...
      if (ignoreIndexItem && fname === "index.md" || /^-.*\.(md|MD)$/.test(fname) || ignoreList.some((item2) => item2 === fname || item2 instanceof RegExp && item2.test(fname))) {
        continue;
      }
